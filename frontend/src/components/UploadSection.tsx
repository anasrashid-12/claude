...
export default function UploadSection() {
  ...
  const { credits, refreshCredits } = useCredits(); // ✅ context credits
      ...
      xhr.onreadystatechange = async () => {
        ...
        try {
          ...
          if (xhr.status >= 200 && xhr.status < 300) {
            ...
            // ✅ Credits refresh via context
            await refreshCredits();

            // ✅ Remove uploaded item from list
            setItems((prev) =>
              prev.filter((p) => p.preview !== fileItem.preview)
            );
  ...
  const handleUploadIndividually = async () => {
    ...
    for (const it of items) {
      ...
      if ((credits ?? 0) <= 0) break; // stop if no credits left
      await uploadOne(it);
    }
    setUploading(false);
  };
      ...
      {items.length > 0 && (
        <div className="space-y-3">
          {items.map((it, idx) => (
            <div
              key={idx}
              className="flex gap-3 items-center p-2 rounded-lg bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700"
            >
