...
def ensure_shop_credits_row(shop_domain: str):
    res = supabase.table("shop_credits") \
        .select("*") \
...
def add_credits_and_record(shop: str, credits_to_add: int, plan_id: str, source: str, purchase_id: str):
    ...
    """
    # idempotency check - safer without maybe_single()
    res = supabase.table("credit_transactions") \
        .select("id") \
        .eq("shop", shop) \
        .eq("external_id", purchase_id) \
        .execute()

    if res.data and len(res.data) > 0:
        return None

    # ensure shop_credits row exists
    res = supabase.table("shop_credits") \
        .select("credits") \
        .eq("shop_domain", shop) \
        .maybe_single() \
    ...
    current_credits = getattr(res, "data", {}).get("credits", 0) if res.data else 0
    ...
    supabase.table("credit_pending").update({
        "status": "COMPLETED",
        "updated_at": now_iso()
